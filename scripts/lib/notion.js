...
const convertToMD = async (pageId, frontmatter="", firstImageAsCover=false) => {
  ...
  if (images?.length) {
    ...
    images.forEach( block => {
      ...
      if (blockIndex >= 0) {
        const { text, url } = getMdImageData(block.parent)
        //console.log("BLOCKS IMAGES ::", text, url)
        //console.log("todo el bloque:", JSON.stringify(mdblocks[blockIndex], null, 4))
        if (text && url) {
        const figure = `<figure><img src="${url}" alt="${text}"><figcaption align="left">${text}</figcaption></figure>`
          mdblocks[blockIndex] = {
            type: 'html',
            parent: figure,
            children: []
        ...
        if (url) {
          frontmatter = addImageToFrontmatter(frontmatter, url)
        }
  ...
  //   children: []
  // }
  const dropboxLinks = mdblocks.filter( b => b.type === 'link_preview' && b.parent.includes('dropbox.com') )
  ...
  if(dropboxLinks?.length){
    ...
    dropboxLinks.forEach( (link, linkIndex) => {
      ...
      if (newL) {
        ...
        if (blockIndex >= 0){ // el indice ha cambiado, no sirve el que tienes
  ...
  // VIDEOS youtube
  const videos = mdblocks.filter( b => b.type === 'video' && b.parent.includes('youtu') )
  if (videos?.length) {
...
function getMdImageData(imageLink){
  //const regex = /!\[(.*?)\]\((.*?)\)/; // no codicioso, termina en el primer ), no en el ultimo
  const regex = /!\[(.*)\]\((.*)\)/
  const myMatch = imageLink.match(regex)
  let full, text, url;
  if (myMatch) { [ full, text, url ] = myMatch }
  return {
      full, text, url
  }
}
...
export const notionPageToMd = async (page, dryrun=true, writeall=false) => {
    ...
    // console.log("page::", response)
      ...
      const keyDate = Object.keys(response.properties).find( k => k.includes("Date") )
      const keyTitle = Object.keys(response.properties).find( k => k.includes("Name") )
      const keyDescription = Object.keys(response.properties).find( k => k.includes("Description") )
    ...
    if(pagesToProcess.length) {
        if (dryrun) console.log("++ DRY RUN page: ", ref, "has", pagesToProcess.length, "files") 
        else console.log("------\n", "++ page: ", ref, "has", pagesToProcess.length, "files") 
...
export const notionDbToMdFiles = async (db, dryrun=true, writeall=false) => {
      ...
      console.log("+ reading Astro content: ", ref, "find ", destFiles.length, "files")
      // articulos en NOTION
      ...
      if (response?.results?.length) {
        // KEYS, es el titulo de la propertie de la base de datos
        const keyDate = Object.keys(response.results[0].properties).find( k => k.includes("Fecha") )
        const keyTitle = Object.keys(response.results[0].properties).find( k => k.includes("Titulo") )
        ...
        const parsed = ready.map( async (p) => {
            ...
            // mismo archivo ya compilado
            const publishedFileIndex = destFiles.findIndex( f => f.data.id == page.id )
            let publishedFile;
      ...
      // y si deben ser publicados o aun no
      if(pagesToProcess?.length) {
        if (dryrun) console.log("++ DRY RUN db: ", ref, "has", pagesToProcess.length, "files") 
        else console.log("------\n", "++ db: ", ref, "has", pagesToProcess.length, "files") 
        // GUARDA una copia de los que sobreescribas, para poder hacer rollback
        pagesToProcess.forEach( md => {
            writeMdFile(md.slug, dest, md.md, dryrun)
            })
      ...
      if (pagesToRemove?.length) {
        if (dryrun) console.log("++ DRY RUN remove: ", ref, "has", pagesToRemove.length, "files to be removed") 
        else {
